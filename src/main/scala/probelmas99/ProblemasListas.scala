...
object ProblemasListas extends App{
  ...
  def myButLast[A](lst:List[A]):A = lst match {
    case x :: _ :: Nil => x
    case _ :: xs => myButLast(xs)
  }
  //println(myButLast(List(1,2,3,4)))
  //println(myButLast(List('x','y','z')))

  ///EXTRA obtener el penultimo y ultimo elemento de una lista
  //Precondicion la lista debe tener al menos 2 elementos
  @tailrec
  def myButLast2[A](lst:List[A]):List[A] = lst match {
    case x :: y :: Nil => List(x,y)
    case _ :: xs => myButLast2(xs)
  }
  //println(myButLast2(List(1,2,3,4)))
  //println(myButLast2(List('x','y','z')))
  ...
  * (element-at '(a b c d e) 3)
  ...
  def myLength[A](lst:List[A]):Int = {
    @tailrec
    def myLengthR(lst:List[A],acc:Int):Int = lst match {
      case Nil => acc
      case _ :: xs => myLengthR(xs,acc+1)
    }
    myLengthR(lst,0)
  }
  ...
  def myReverse[A](lst:List[A]):List[A] = {
    @tailrec
    def myReverseR(lst:List[A],acc:List[A]):List[A] = lst match {
      case Nil => acc
      case x :: xs => myReverseR(xs, x :: acc)
    }
    myReverseR(lst,Nil)
  }
